...
def probe_openalex(doi: str = "", title: str = "", openalex_id: str = "", **kw) -> dict | None:
...
def probe_core(doi: str, title: str = "", openalex_id: str = "", **kw) -> dict | None:
    """
    Query CORE API v3 for OA repository records and deposit dates.
    Requires a free API key (set CORE_API_KEY env var).
    No full text downloaded.
    """
...
def main():
    ...
    print(f"Random sample: {n} articles (seed={RANDOM_SEED})\n")
    ...
    with open(OUTPUT_CSV, mode, newline="", encoding="utf-8") as csvfile:
        ...
        for idx in range(start_idx, n):
            ...
            for key in enabled_keys:
                ...
                r = probe_fn(doi=doi, title=title, openalex_id=openalex_id)
                if r:
